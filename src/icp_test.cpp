...
private:
    ...
    int seq;
    pcl::PointCloud<PointLidar>::Ptr cloud_all;
public:
    Node() {
        ...
        seq = 0;
        cloud_all = boost::make_shared<pcl::PointCloud<PointLidar>>();
    };
...
//            std::cout << "todo\n";
    ...
    icp(pcl::PointCloud<PointLidar>::Ptr &cloud_from, pcl::PointCloud<PointLidar>::Ptr &cloud_to) {
        pcl::IterativeClosestPoint<PointLidar, PointLidar> icp; // 使用的是 gicp
...
//                continue;
        ...
        this->cloud_all->clear();
        auto cloud_first = *this->cloud_queue.begin();
        auto cloud_last = *(this->cloud_queue.end()-1);
        for (auto p : cloud_first->points)
            this->cloud_all->push_back(p);
        this->transform(cloud_last, cloud_first);
        for (auto p : cloud_last->points)
            this->cloud_all->push_back(p);

        pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_show(new pcl::PointCloud<pcl::PointXYZRGB>);
        for (auto p : this->cloud_all->points) {
            pcl::PointXYZRGB tmp;
            tmp.x = p.x, tmp.y = p.y, tmp.z = p.z;
