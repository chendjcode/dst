...
//
/*
 * 我打算将点云以八叉树划分，选取叶子中的一点来代表这个叶子的 eou
 * 计算 eou 使用 kdtree， kdtree 以 pox, poy, poz 进行创建
 * 一致性评估同样使用 kdtree，同上
 * */
...
//            std::cout << "todo\n";
    ...
    dst(pcl::PointCloud<PointLidar>::Ptr &cloud) {
        ...
        for (auto it = octree.leaf_begin(); it != octree.leaf_end(); it++) {
            ...
            auto idx_s = this->kdtree_search(kdtree, P);
            for (auto idx_ : idx_s) { // 仅计算体素中的一个点的dst
    ...
    consistency_assessment(pcl::PointCloud<PointLidar>::Ptr &cloud_k, pcl::PointCloud<PointLidar>::Ptr &cloud_ref) {
        pcl::octree::OctreePointCloud<PointLidar> octree(0.2f); // 设置八叉树的分辨率为 0.4 m
